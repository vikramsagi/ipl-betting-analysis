...
#1.	Show the percentage of wins of each bidder 
# in the order of highest to lowest percentage.

select 
bidder_id Id ,bidder_name name,
sum(case
...
using(user_id)
join ipl_bidding_details ibid using(bidder_id)
...
select stadium_id,stadium_name,city,count(city) no_of_matches
from ipl_match_schedule left join ipl_match using(match_id)
...
group by it.team_name with rollup;

#5.	Show the team id who won the match as per the win details.

select  it.team_id,it.remarks winning_team_name,win_details 
from ipl_match join ipl_team it on 
substr(ipl_match.win_details,6,2)
=substr(it.remarks,1,2)
order by team_id,team_name;

#6.	Display total matches played, 
# total matches won and total matches lost by team 
# along with its team name.

select team_id,team_name,sum(matches_played) matches_played,sum(matches_won) matches_won,sum(matches_lost) matches_lost,
sum(no_result) no_result 
from ipl_team_standings join ipl_team
using(team_id)
group by team_id;
...
select player_name,player_id,player_role,substr(itp.remarks,8,2) Team 
from ipl_team_players itp
join ipl_player using(player_id)
where player_role="bowler"
and substr(itp.remarks,8,2)="MI";
...
# all-rounders in each team
select substr(remarks,7) team_name,count(remarks) No_of_all_rounders from ipl_team_players
where player_role="All-Rounder"
group by remarks
order by count(remarks);

# having more than 4 all-rounders
select substr(remarks,7) team_name,count(remarks) No_of_all_rounders from ipl_team_players
where player_role="All-Rounder"
group by remarks
having count(remarks)>4
...
select * from ipl_team order by team_id;
select distinct bid_team from ipl_bidding_details;
select * from ipl_team_standings;
